...
namespace Neo.UnitTests.Ledger
{
    ...
    public class UT_TransactionVerificationContext
    {
        ...
        public async Task TestDuplicateOracle()
        {
            ...
            Assert.IsTrue(verificationContext.CheckTransaction(tx, conflicts, snapshotCache, TestBlockchain.TheNeoSystem.NativeContractRepository));
            verificationContext.AddTransaction(tx);

            tx = CreateTransactionWithFee(2, 1);
            tx.Attributes = new TransactionAttribute[] { new OracleResponse() { Code = OracleResponseCode.ConsensusUnreachable, Id = 1, Result = Array.Empty<byte>() } };
            Assert.IsFalse(verificationContext.CheckTransaction(tx, conflicts, snapshotCache, TestBlockchain.TheNeoSystem.NativeContractRepository));
        }
        ...
        public async Task TestTransactionSenderFeeWithConflicts()
        {
            ...
            Assert.IsTrue(verificationContext.CheckTransaction(tx, conflicts, snapshotCache, TestBlockchain.TheNeoSystem.NativeContractRepository)); // 1 GAS is left on the balance + 2 GAS is free after conflicts removal => enough for one more trasnaction.
        }
    }
}
