    ...
    public partial class UT_InteropService : TestKit
    {
        ...
        public void Runtime_GetNotifications_Test()
        {
            ...
            using (var script = new ScriptBuilder())
            {
                ...
                snapshotCache.DeleteContract(scriptHash2, TestBlockchain.TheNeoSystem.NativeContractRepository);
                ...
                snapshotCache.AddContract(scriptHash2, contract, TestBlockchain.TheNeoSystem.NativeContractRepository);
            }
        ...
        public void TestExecutionEngine_GetCallingScriptHash()
        {
            ...
            engine.SnapshotCache.AddContract(contract.Hash, contract, TestBlockchain.TheNeoSystem.NativeContractRepository);
        ...
        public void TestBlockchain_GetHeight()
        {
            ...
            Assert.AreEqual((uint)0, TestBlockchain.TheNeoSystem.NativeContractRepository.Ledger.CurrentIndex(engine.SnapshotCache));
        }
        ...
        public void TestBlockchain_GetBlock()
        {
            ...
            Assert.IsNull(TestBlockchain.TheNeoSystem.NativeContractRepository.Ledger.GetBlock(engine.SnapshotCache, UInt256.Zero));

            var data1 = new byte[] { 0x01, 0x01, 0x01 ,0x01, 0x01, 0x01, 0x01, 0x01,
                                        0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
                                        0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
                                        0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01};
            Assert.IsNull(TestBlockchain.TheNeoSystem.NativeContractRepository.Ledger.GetBlock(engine.SnapshotCache, new UInt256(data1)));
            Assert.IsNotNull(TestBlockchain.TheNeoSystem.NativeContractRepository.Ledger.GetBlock(engine.SnapshotCache, TestBlockchain.TheNeoSystem.GenesisBlock.Hash));
        }
        ...
        public void TestBlockchain_GetTransaction()
        {
            ...
            var data1 = new byte[] { 0x01, 0x01, 0x01 ,0x01, 0x01, 0x01, 0x01, 0x01,
                                        ...
                                        0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
                                        0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01};
            Assert.IsNull(TestBlockchain.TheNeoSystem.NativeContractRepository.Ledger.GetTransaction(engine.SnapshotCache, new UInt256(data1)));
        }
        ...
        public void TestBlockchain_GetTransactionHeight()
        {
            ...
            using var script = new ScriptBuilder();
            script.EmitDynamicCall(TestBlockchain.TheNeoSystem.NativeContractRepository.Ledger.Hash, "getTransactionHeight", state.Transaction.Hash);
            engine.LoadScript(script.ToArray());
            engine.Execute();
        ...
        public void TestBlockchain_GetContract()
        {
            ...
            var data1 = new byte[] { 0x01, 0x01, 0x01 ,0x01, 0x01,
                                        ...
                                        0x01, 0x01, 0x01, 0x01, 0x01,
                                        0x01, 0x01, 0x01, 0x01, 0x01 };
            Assert.IsNull(TestBlockchain.TheNeoSystem.NativeContractRepository.ContractManagement.GetContract(engine.SnapshotCache, new UInt160(data1)));

            var state = TestUtils.GetContract();
            engine.SnapshotCache.AddContract(state.Hash, state, TestBlockchain.TheNeoSystem.NativeContractRepository);
            engine = ApplicationEngine.Create(TriggerType.Application, null, engine.SnapshotCache, TestBlockchain.TheNeoSystem.NativeContractRepository);
            engine.LoadScript(new byte[] { 0x01 });
            Assert.AreEqual(state.Hash, TestBlockchain.TheNeoSystem.NativeContractRepository.ContractManagement.GetContract(engine.SnapshotCache, state.Hash).Hash);
        }
        ...
        {
            var engine = GetEngine(true, true);
            var contract = TestBlockchain.TheNeoSystem.NativeContractRepository.ContractManagement.GetContractById(engine.SnapshotCache, -1);
        ...
        {
            var engine = GetEngine(true, true);
            Assert.IsTrue(TestBlockchain.TheNeoSystem.NativeContractRepository.ContractManagement.HasMethod(engine.SnapshotCache, TestBlockchain.TheNeoSystem.NativeContractRepository.NEO.Hash, "symbol", 0));
            Assert.IsTrue(TestBlockchain.TheNeoSystem.NativeContractRepository.ContractManagement.HasMethod(engine.SnapshotCache, TestBlockchain.TheNeoSystem.NativeContractRepository.NEO.Hash, "transfer", 4));
        }
        ...
        {
            var engine = GetEngine(true, true);
            var list = TestBlockchain.TheNeoSystem.NativeContractRepository.ContractManagement.ListContracts(engine.SnapshotCache);
            ...
            var list2 = TestBlockchain.TheNeoSystem.NativeContractRepository.ContractManagement.ListContracts(engine.SnapshotCache);
        ...
        public void TestStorage_Put()
        {
            ...
            snapshotCache.AddContract(state.Hash, state,TestBlockchain.TheNeoSystem.NativeContractRepository);
            snapshotCache.Add(storageKey, storageItem);
            engine = ApplicationEngine.Create(TriggerType.Application, null, snapshotCache,TestBlockchain.TheNeoSystem.NativeContractRepository);
            engine.LoadScript(new byte[] { 0x01 });
            key = new byte[] { 0x01 };
            value = new byte[] { 0x02 };
        ...
        public void TestContract_Call()
        {
            ...
            engine.SnapshotCache.DeleteContract(state.Hash, TestBlockchain.TheNeoSystem.NativeContractRepository);
            engine.SnapshotCache.AddContract(state.Hash, state, TestBlockchain.TheNeoSystem.NativeContractRepository);
        ...
        public void TestContract_Destroy()
        {
            ...
            snapshotCache.AddContract(scriptHash, state,TestBlockchain.TheNeoSystem.NativeContractRepository);
            snapshotCache.Add(storageKey, storageItem);
            snapshotCache.DestroyContract(scriptHash, TestBlockchain.TheNeoSystem.NativeContractRepository);
        ...
        private ApplicationEngine GetEngine(bool hasContainer = false, bool hasBlock = false, bool addScript = true, long gas = 20_00000000)
        {
            ...
            var engine = ApplicationEngine.Create(TriggerType.Application, tx, snapshot, TestBlockchain.TheNeoSystem.NativeContractRepository, block, TestBlockchain.TheNeoSystem.Settings, gas: gas);
        ...
        public void TestGetBlockHash()
        {
            ...
            var hash = TestBlockchain.TheNeoSystem.NativeContractRepository.Ledger.GetBlockHash(snapshotCache, 0);
            var hash2 = TestBlockchain.TheNeoSystem.NativeContractRepository.Ledger.GetBlock(snapshotCache, 0).Hash;
            var hash3 = TestBlockchain.TheNeoSystem.NativeContractRepository.Ledger.GetHeader(snapshotCache, 0).Hash;
            ...
            Assert.IsTrue(TestBlockchain.TheNeoSystem.NativeContractRepository.Ledger.ContainsBlock(snapshotCache, hash));
        }
        ...
        {
            var snapshotCache = GetEngine(true, true).SnapshotCache;
            var vote = TestBlockchain.TheNeoSystem.NativeContractRepository.NEO.GetCandidateVote(snapshotCache, new ECPoint());
        ...
        public void TestContractPermissionDescriptorEquals()
        {
            ...
            var descriptor2 = ContractPermissionDescriptor.Create(TestBlockchain.TheNeoSystem.NativeContractRepository.NEO.Hash);
