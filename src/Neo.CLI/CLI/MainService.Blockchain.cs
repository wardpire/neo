...
namespace Neo.CLI
{
    partial class MainService
    {
        ...
        private void OnShowBlockCommand(string indexOrHash)
        {
            lock (syncRoot)
            {
                ...
                if (uint.TryParse(indexOrHash, out var index))
                    block = NeoSystem.NativeContractRepository.Ledger.GetBlock(NeoSystem.StoreView, index);
                else if (UInt256.TryParse(indexOrHash, out var hash))
                    block = NeoSystem.NativeContractRepository.Ledger.GetBlock(NeoSystem.StoreView, hash);
                else
                {
                ...
                ConsoleHelper.Info("", "  PrimaryPubKey: ", $"{NeoSystem.NativeContractRepository.NEO.GetCommittee(NeoSystem.GetSnapshotCache())[block.PrimaryIndex]}");
        ...
        public void OnShowTransactionCommand(UInt256 hash)
        {
            lock (syncRoot)
            {
                var tx = NeoSystem.NativeContractRepository.Ledger.GetTransactionState(NeoSystem.StoreView, hash);
                ...
                var block = NeoSystem.NativeContractRepository.Ledger.GetHeader(NeoSystem.StoreView, tx.BlockIndex);
        ...
        public void OnShowContractCommand(string nameOrHash)
        {
            lock (syncRoot)
            {
                ...
                if (UInt160.TryParse(nameOrHash, out var scriptHash))
                    contract = NeoSystem.NativeContractRepository.ContractManagement.GetContract(NeoSystem.StoreView, scriptHash);
                else
                {
                    var nativeContract = NeoSystem.NativeContractRepository.Contracts.SingleOrDefault(s => s.Name.Equals(nameOrHash, StringComparison.InvariantCultureIgnoreCase));

                    if (nativeContract != null)
                        contract = NeoSystem.NativeContractRepository.ContractManagement.GetContract(NeoSystem.StoreView, nativeContract.Hash);
                }
