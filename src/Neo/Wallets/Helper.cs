...
namespace Neo.Wallets
{
    ...
    public static class Helper
    {
        ...
        public static long CalculateNetworkFee(this Transaction tx, DataCache snapshot, ProtocolSettings settings, NativeContractRepository nativeContractRepository, Wallet? wallet = null, long maxExecutionCost = ApplicationEngine.TestModeGas)
        {
            Func<UInt160, byte[]?>? accountScript = wallet != null ? (scriptHash) => wallet.GetAccount(scriptHash)?.Contract?.Script : null;
            return CalculateNetworkFee(tx, snapshot, settings, nativeContractRepository, accountScript, maxExecutionCost);
        }
        ...
        {
            var hashes = tx.GetScriptHashesForVerifying(snapshot, nativeContractRepository);
            ...
            var exec_fee_factor = nativeContractRepository.Policy.GetExecFeeFactor(snapshot);
            long networkFee = 0;
