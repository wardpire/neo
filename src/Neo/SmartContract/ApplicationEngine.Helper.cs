    ...
    public partial class ApplicationEngine : ExecutionEngine
    {
        public string GetEngineStackInfoOnFault(bool exceptionStackTrace = true, bool exceptionMessage = true)
        {
            ...
            if (CallingScriptHash != null)
                traceback.AppendLine($"CallingScriptHash={CallingScriptHash}[{NativeContractRepository.ContractManagement.GetContract(SnapshotCache, CallingScriptHash)?.Manifest.Name}]");
            traceback.AppendLine($"CurrentScriptHash={CurrentScriptHash}[{NativeContractRepository.ContractManagement.GetContract(SnapshotCache, CurrentScriptHash)?.Manifest.Name}]");
            ...
            foreach (ExecutionContext context in InvocationStack.Reverse())
            {
                ...
                string contextContractName = NativeContractRepository.ContractManagement.GetContract(SnapshotCache, contextScriptHash)?.Manifest.Name;
