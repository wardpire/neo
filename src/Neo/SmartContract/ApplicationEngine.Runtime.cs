    ...
    partial class ApplicationEngine
    {
        ...
        protected internal bool CheckWitnessInternal(UInt160 hash)
        {
            ...
            if (ScriptContainer is Transaction tx)
            {
                ...
                {
                    OracleRequest request = NativeContractRepository.Oracle.GetRequest(SnapshotCache, response.Id);
                    signers = NativeContractRepository.Ledger.GetTransaction(SnapshotCache, request.OriginalTxid).Signers;
                }
            ...
            return ScriptContainer.GetScriptHashesForVerifying(SnapshotCache, NativeContractRepository).Contains(hash);
        }
